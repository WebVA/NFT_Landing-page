...
CACHINGDIST=`cat package.json | jq -r '.cachingdist'` || exit 1
[[ -z $CACHINGDIST ]] && {
    CACHINGDIST="NONE"
}
...
echo "Caching distribution:  $CACHINGDIST"
echo "S3 Buckets:"
...
[[ $CACHINGDIST != "NONE" ]] && {
    ...
    ID=`aws cloudfront create-invalidation \
        --profile $PROFILE \
        --distribution-id $CACHINGDIST \
        --paths "/*" \
    | jq -r '.Invalidation.Id'`
    [[ -z $ID ]] && {
        echo "Could not send invalidation"
        exit 1
    }
    echo "Invalidation id $ID has been sent, note that propagation can take 5-10min."
}
